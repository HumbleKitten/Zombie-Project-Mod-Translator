...
public class Translation {
    ...
    public static Integer b1(List<File> allFileList, String translation, JTextPaneA jTextPane, IdKey idKey) {
        Integer num = 1;
        Integer length = 0;
        ...
        for (File file : allFileList) {
            ...
            jTextPane.setText(jTextPane.getText()
                    +
                    "第[" + num + "/" + allFileList.size() + "]个文件:" + file.getName() + "开始翻译" +
                    "\n"
            );
            ...
            try {
                ...
                while (sc.hasNextLine()) {  //按行读取字符串
                    line = sc.nextLine();
                    if (line.contains("=") && (line.contains(",") || line.contains("\"")) && !line.contains(file.getName().split("\\.")[0])) {
                        String query = null;
                        String name = null;
                        if (line.contains(",")) {
            ...
            jTextPane.setText(jTextPane.getText()
                    +
                    "第[" + num + "/" + allFileList.size() + "]个文件:" + file.getName() + "翻译完成,翻译次数:" + index + ",耗时:" + (double) (endTime - currentTime) / 1000 + "秒,本文件翻译字符数:" + (length - lengthItem) +
                    "\n"
            );
            num++;
        }
        jTextPane.setText(jTextPane.getText()
                +
                "翻译文件共有" + allFileList.size() + "个文件,已全部翻译,翻译总字符数:" + length +
                "\n"
        );
        return length;
    }
